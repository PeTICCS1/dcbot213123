...
exports.start = function(SETUP) {
  ...
  const sendOrUpdate = function(embed) {
    if (MESSAGE !== undefined) {
      MESSAGE.edit(embed).then(() => {
        log(LOG_LEVELS.DEBUG,'Sikeres Frissítés!');
      }).catch(() => {
        log(LOG_LEVELS.ERROR,'Sikertelen Frissítés!');
      })
    } else {
      ...
      if (channel !== undefined) {
        channel.fetchMessage(MESSAGE_ID).then((message) => {
          MESSAGE = message;
          message.edit(embed).then(() => {
            log(LOG_LEVELS.SPAM,'A Frissítés Sikeres!');
          }).catch(() => {
            log(LOG_LEVELS.ERROR,'A Frissítés Sikertelen!');
          });
        }).catch(() => {
          channel.send(embed).then((message) => {
            MESSAGE = message;
            log(LOG_LEVELS.INFO,`Az üzenet le lett küldve; (${message.id})`);
          }).catch(console.error);
        })
      } else {
        log(LOG_LEVELS.ERROR,'Nincs Frissítési szoba beállítva!');
      }
    }
  };
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("Nirvana RolePlay RolePlay Státusz", "https://cdn.discordapp.com/attachments/1061307399610302584/1061313266594353212/logo.jpg")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Nirvana RolePlay' : '⚫ Nirvana RolePlay')
    .setTimestamp(new Date())
    .addField("Információk:", `‎`)
    .addField("Amennyiben úgy érzed szeretnél egy kedves, összetartó csapattal játszani,", `**Akkor itt az ideje hogy fel csatlakozz a szerverünkre!**`)
    .addField("Discord Szerver Link:", `https://discord.gg/nirvanaroleplay`)
    .addField("Ha te is csatlakozni szeretnél a szerver közösségéhez,", `FiveM szerver kereső -> Nirvana RolePlay`)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Szerver státusz:',`${STATUS}\n\u200b\n`);
  ...
  const offline = function() {
    ...
    if (LAST_COUNT !== null) log(LOG_LEVELS.INFO,`A Szerver Offline Állapotban van. ${URL_SERVER} (${URL_PLAYERS} ${URL_INFO})`);
    let embed = UpdateEmbed()
    .setColor(0xff0000)
    .addField('Szerver Státusz:',':exclamation: A Szerver Nem Elérhető! :exclamation: ',true)
    .addField('Várólista:',':x:',true)
    .addField('Elérhető Játékosok:',':x:\n\u200b\n',true);
    sendOrUpdate(embed);
    LAST_COUNT = null;
  };
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        let embed = UpdateEmbed()
        .addField('Szerver Státusz:',':white_check_mark: Elérhető',true)
        .addField('Várólista:',queue === 'Bekapcsolva' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Elérhető Játékosok:',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
    ...
    return new Promise((resolve,reject) => {
      ...
      let id = loop_callbacks.push(() => {
        if (!resolved) {
          ...
          log(LOG_LEVELS.ERROR,'Loop Callback Időtullépés');
          reject(null);
        }
      })
      setTimeout(() => {
  ...
  bot.on('Lecsatlakozott',(devent,shard) => {
    ...
    checkLoop().then((running) => {
      log(LOG_LEVELS.INFO,`A loop működése: ${running}`);
    }).catch(console.error);
  })

  bot.on('Újracsatlakozás',(shard) => {
    log(LOG_LEVELS.INFO,'Újracsatlakozás');
    checkLoop().then((running) => {
      log(LOG_LEVELS.INFO,`A loop működése: ${running}`);
    }).catch(console.error);
  })

  bot.on('Folyatás',(replayed,shard) => {
  ...
  bot.on('rateLimit',(info) => {
    ...
    if (info.path.startsWith(`/channels/${CHANNEL_ID}/messages/${MESSAGE_ID ? MESSAGE_ID : MESSAGE ? MESSAGE.id : ''}`)) bot.emit('Újraindítás');
    checkLoop().then((running) => {
      log(LOG_LEVELS.DEBUG,`A loop működése: ${running}`);
    }).catch(console.error);
  })
      ...
      if (message.member) {
        if (message.member.hasPermission('ADMINISTRATOR')) {
          if (message.content.startsWith('+status ')) {
            ...
            .setColor(0x2894C2)
            .setTitle('Státusz frissítve!')
            .setTimestamp(new Date());
            if (status === 'clear') {
              STATUS = undefined;
              embed.setDescription('A státusz törlésre került!');
            } else {
              STATUS = status;
              embed.setDescription(`Új elérhető státusz:\n\`\`\`${STATUS}\`\`\``);
            }
            bot.channels.get(LOG_CHANNEL).send(embed);
            return log(LOG_LEVELS.INFO,`${message.author.username} frissítette a státuszt!`);
          }
        }
        if (message.channel.id === SUGGESTION_CHANNEL) {
          ...
          message.channel.send(embed).then((message) => {
            ...
            sent.react('👍').then(() => {
              sent.react('👎').then(() => {
                log(LOG_LEVELS.SPAM,'Egy ötlet elküldésre került!');
              }).catch(console.error);
            }).catch(console.error);
          }).catch(console.error);
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setAuthor(message.member.nickname ? message.member.nickname : message.author.tag,message.author.displayAvatarURL)
          .setColor(0x2894C2)
          .setTitle('Hiba jelentés')
          .setDescription('A hiba jelentésedet rögzítettük!')
          .setTimestamp(new Date());
  ...
  bot.login(BOT_TOKEN).then(null).catch(() => {
    log(LOG_LEVELS.ERROR,'Sikertelen csatlakozás, próbáld újra!');
    console.error(e);
    process.exit(1);
  });
